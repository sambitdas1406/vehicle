...
<body>
	<div id="wrapper">
		...
		<div id="page-wrapper">
			...
			<div id="page-inner">
								...
								<a href="LeaveEmployee.jsp" class="waves-effect waves-light btn green" style="float: right; margin-right:60px;">Add Leave Employee</a>
    ...
    <h3 align="center"style="color:#424bf4;"class="well">LEAVE  INFORMATION</h3>
<%!
...
String sqlPgintn="SELECT SQL_CALC_FOUND_ROWS * FROM vehicle.leave limit "+iPagNo+","+iSwRws+"";
...
<TD><%=rsPgin.getDate(4)%></TD>
<TD><%=rsPgin.getDate(5)%></TD>
<TD><%=rsPgin.getString(6)%></TD>
  <td style="padding:12px 20px;    margin-right: 10px;"class="btn orange"><a href="UpdateLeaveEmployee.jsp?id=<%=rsPgin.getString("id")%>"><i style="color:#ffff" class="fa fa-edit" ></i></a></td>
...
%>
<a href="ViewLeaveEmployee.jsp?iPagNo=<%=prePageNo%>&cPagNo=<%=prePageNo%>"><< Previous</a>
	...
	for(i=((cPge*iTotSrhRcrds)-(iTotSrhRcrds-1));i<=(cPge*iTotSrhRcrds);i++)
    {
		if(i==((iPagNo/iSwRws)+1))
      	{
      	%>
<a href="ViewLeaveEmployee.jsp?iPagNo=<%=i%>" style="cursor:pointer;color:red"><b><%=i%></b></a>
...
<%
		...
		{ 
		%> 
		<a href="ViewLeaveEmployee.jsp?iPagNo=<%=i%>"><%=i%></a> 
		<% 
		}
} 
	...
	{ 
	%>
	 <a href="ViewLeaveEmployee.jsp?iPagNo=<%=i%>&cPagNo=<%=i%>">>> Next</a> 
	 <%
	 }
} 
%> 
